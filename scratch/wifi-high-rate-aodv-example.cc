...
NS_LOG_COMPONENT_DEFINE("Wifi-High-Rate-AODV-Example");
...
void 
AODVExample::PrintTime()
{
    std::cout << "Time: " << Simulator::Now().GetSeconds() << "\n";
    Simulator::Schedule(Seconds(1.0), &AODVExample::PrintTime, this);
...
AODVExample::Run()
{
    ...
    std::ofstream flowOut("TaskA1/" + m_prefix +"-flowstats");
...
AODVExample::InstallApplications()
{
    ...
    std::ofstream serverClient("TaskA1/" + m_prefix + "-server-client.txt");
    ...
    // move the first client away after 5 seconds
    if(m_moveClients){
       
        int c = m_clients[0];
        Ptr<Node> node = nodes.Get(c);
        Ptr<MobilityModel> mob = node->GetObject<MobilityModel> ();
        Simulator::Schedule(Seconds(m_totalTime/3), &MobilityModel::SetPosition, mob, Vector(1000, 1000, 1000));
